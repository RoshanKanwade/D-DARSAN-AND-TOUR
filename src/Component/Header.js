...
function Header() {
  ...
  const selectFun=()=> {
   ...
   for (var i=0; i < state.states.length; i++){
     if(state.states[i].state==x){
      for (var j=0; j < state.states[i].state.length; j++){
     str+="<option value='"+state.states[i].districts[j]+"'>"+state.states[i].districts[j]+"</option>"
     }
    }
  }
    str+=" </select> <button type='button' onClick={selectFun}>search</button></form></>"
  ...
  return (<>
    <Navbar className="color-nav" expand="lg" fixed="top"  >
      <Container fluid>
        <Navbar.Brand >
          <img className="logo" src="./CompanyLogo.jpg"></img></Navbar.Brand>
...

            {
              userrole && (
                <NavDropdown title="Support" id="navbarScrollingDropdown">
                  ...
                  <NavDropdown.Item href="#action4">Booking Details</NavDropdown.Item>
